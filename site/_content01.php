		...
		private function p($p){
			...
			if(!isset($_SESSION['logat'])){
				if($p != 'autentificare' && $p != 'logout'){
					echo "<META http-equiv='refresh' content=\"0;URL=http://www.mateonline.net".DS.APP_ROOT.DS."/administrare/index.php?p=autentificare\">";
				}
			} elseif($_SESSION['logat'] != 1){
				if($p != 'autentificare' && $p != 'logout'){
					echo "<META http-equiv='refresh' content=\"0;URL=index.php?p=autentificare\">";
				}
			} */
		}
}

$content01 = new Content01($common->pagina);		//	am initializat si clasa aici. Deci, gata!
